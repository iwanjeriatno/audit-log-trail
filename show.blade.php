                ...
                @slot('tbody')
                    @foreach ($logs as $key => $value)
                        <tr>
                            ...
                            {{-- <td class="text-center">
                              @component('_components.actions')
                                  @if(RolePerms::isShow('pengaturan'))
                                      @slot('show') {{ route('menu.show', $value->id) }} @endslot
                                  @endif
                                  @if(RolePerms::isEdit('pengaturan'))
                                      @slot('edit') {{ route('menu.edit', $value->id) }} @endslot
                                  @endif
                                  @if(RolePerms::isDelete('pengaturan'))
                                      @slot('destroy') {{ route('menu.destroy', $value->id) }} @endslot
                                  @endif
                              @endcomponent
                            </td> --}}
                        </tr>
